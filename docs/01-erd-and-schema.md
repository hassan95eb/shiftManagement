...
سقف ماهانه‌ی Downtime نیز بر اساس **ماه جلالی تهران** و همین مرز ثابت UTC+03:30
محاسبه می‌شود؛ هیچ مرز ماه میلادی یا تاریخ UTC در این محاسبه دخالت ندارد.

تأیید مرخصی، شیفت را `Released` می‌کند ولی `AssignedCallAgentId` را روی کارشناسِ
مرخصی نگه می‌دارد. V6 هنگام پرکردن شیفت با Cover این اشاره‌گر را بازنویسی می‌کند؛
بنابراین V8 باید ساعت‌های متعهد و زمان معاف‌شده را از خود ردیف `AgentRequests`
استخراج کند، نه از تخصیص فعلی شیفت.

محدودیت شناخته‌شده: دو تأیید هم‌زمان روی **دو شیفت متفاوت** می‌توانند هر دو
مانده‌ی مرخصی `1` را بخوانند. `UX_AgentRequests_OneApprovedLeave` فقط رقابت دو
مرخصی روی یک شیفت را می‌بندد؛ سریال‌سازی سهمیه در مقیاس چند پردازش خارج از Scope
این نسخه است.
