...
1. **Ratings, not CallAgentRatings.** A mechanical rename turns `ExpertRating`
   into `CallAgentRating` and the table into `CallAgentRatings`. That is wrong:
   the entity is `Rating`, the table is `Ratings`, the FK column is
   `CallAgentId`, and the unique constraint is `UQ_Ratings_CallAgent_Period`.
   Same for the configuration class: `ExpertRatingConfiguration` →
   `RatingConfiguration`. `ExpertProject` → `CallAgentProject` /
   `CallAgentProjects` is the plain rename and needs no special handling.
2. **Migration history is immutable.** `20260909225615_InitialCreate.cs` and
   its `.Designer.cs` legitimately contain the old names and are not edited —
   editing them would break the migration chain against any database that
   already ran them. `AppDbContextModelSnapshot.cs` is different: it is
   regenerated by the new migration and carries the new names.
