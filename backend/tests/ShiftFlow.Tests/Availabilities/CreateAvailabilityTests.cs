...
public class CreateAvailabilityTests
{
    ...
    public async Task Creating_a_first_window_stores_and_returns_it()
    {
        ...
        Assert.Equal([(At(8), At(12))], StoredWindows(ctx, callAgent.Id));
    }
    ...
    public async Task Posting_adjacent_windows_forward_order_stores_one_merged_window()
    {
        using var ctx = new SqliteTestContext();
        var callAgent = ctx.Db.AddCallAgent("Jane Doe");
        var service = ServiceFor(ctx, callAgent.UserId, callAgent.Id);
    ...
    public async Task A_window_spanning_several_existing_ones_collapses_them_to_one()
    {
        using var ctx = new SqliteTestContext();
        var callAgent = ctx.Db.AddCallAgent("Jane Doe");
        ctx.Db.AddAvailability(callAgent.Id, At(9), At(10));
        ctx.Db.AddAvailability(callAgent.Id, At(11), At(12));
        ctx.Db.AddAvailability(callAgent.Id, At(13), At(14));

        await ServiceFor(ctx, callAgent.UserId, callAgent.Id)
            .CreateAsync(Request(7, 18), CancellationToken.None);

        Assert.Equal([(At(7), At(18))], StoredWindows(ctx, callAgent.Id));
    }
    ...
    public async Task A_non_adjacent_window_is_stored_alongside_the_existing_one()
    {
        ...
        Assert.Equal([(At(8), At(10)), (At(14), At(16))], StoredWindows(ctx, callAgent.Id));
    }
    ...
    public async Task A_window_already_inside_an_existing_one_writes_nothing_and_returns_the_host()
    {
        using var ctx = new SqliteTestContext();
        var callAgent = ctx.Db.AddCallAgent("Jane Doe");
        var host = ctx.Db.AddAvailability(callAgent.Id, At(8), At(18));

        var returned = await ServiceFor(ctx, callAgent.UserId, callAgent.Id)
            .CreateAsync(Request(10, 12), CancellationToken.None);

        Assert.Equal(host.Id, returned.Id);
        Assert.Equal([(At(8), At(18))], StoredWindows(ctx, callAgent.Id));
    }
}
