...
public class UpdateAvailabilityTests
{
    ...
    public async Task Extending_a_window_until_it_touches_a_neighbour_merges_the_two()
    {
        ...
        var morning = ctx.Db.AddAvailability(expert.Id, At(8), At(10));
        ctx.Db.AddAvailability(expert.Id, At(14), At(16));

        var result = await ServiceFor(ctx, expert.UserId, expert.Id)
    ...
    public async Task Shrinking_a_window_that_still_covers_its_approved_shift_is_allowed()
    {
        ...
        Assert.Equal([(At(8), At(13))], StoredWindows(ctx, expert.Id));
    }
    ...
    public async Task Resizing_a_window_so_it_no_longer_covers_an_approved_shift_is_blocked()
    {
        ...
        Assert.Equal([(At(6), At(20))], StoredWindows(ctx, expert.Id));
    }

    [Fact]
    public async Task Updating_another_experts_window_is_NotFound_and_changes_nothing()
    {
        using var ctx = new SqliteTestContext();
