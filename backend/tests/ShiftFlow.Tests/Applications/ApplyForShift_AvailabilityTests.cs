...
public class ApplyForShift_AvailabilityTests
{
    ...
    private static ApplicationService ApplyService(SqliteTestContext ctx, int userId, int expertId) =>
        new(ctx.Db, StubCurrentUser.Expert(userId, expertId), new TestClock(Now));

    private static AvailabilityService AvailabilityServiceFor(SqliteTestContext ctx, int userId, int expertId) =>
        new(ctx.Db, StubCurrentUser.Expert(userId, expertId), new TestClock(Now));
    ...
    public async Task A_window_that_covers_the_whole_shift_lets_the_application_through()
    {
        ...
        ctx.Db.AddAvailability(expert.Id, At(8), At(16)); // exact cover, endpoints included

        var created = await ApplyService(ctx, expert.UserId, expert.Id)
    ...
    public async Task A_window_that_only_partly_covers_the_shift_is_rejected()
    {
        ...
        var expert = ctx.Db.AddExpert("Jane Doe");
        ctx.Db.Assign(expert.Id, project.Id);
        ctx.Db.AddAvailability(expert.Id, At(8), At(14)); // ends two hours short

        await Assert.ThrowsAsync<BusinessRuleViolationException>(() =>
            ApplyService(ctx, expert.UserId, expert.Id).ApplyAsync(shift.Id, CancellationToken.None));
    ...
    public async Task Two_windows_that_only_together_span_the_shift_but_do_not_touch_are_rejected()
    {
        ...
        ctx.Db.AddAvailability(expert.Id, At(8), At(12));
        ctx.Db.AddAvailability(expert.Id, At(13), At(16));
    ...
    public async Task A_shift_spanning_a_merged_availability_boundary_is_accepted()
    {
        ...
        var availability = AvailabilityServiceFor(ctx, expert.UserId, expert.Id);
