...
/// <summary>
/// The one mutation an employer may make to an existing shift: correcting the
...
public class UpdateShiftTests
{
    ...
    [Fact]
    public async Task Editing_another_employers_shift_is_NotFound()
    {
        using var ctx = new SqliteTestContext();
        var acme = ctx.Db.AddEmployer("Acme");
